...
%
\title{\IMSeventname\ Template V4c in US Letter Page Size}
%
...
J. Clerk Maxwell\IMSauthorrefmark{\#1},
Charles Darwin\IMSauthorrefmark{\$},
Michael Faraday\IMSauthorrefmark{*\textasciicircum2}, 
Andr{\'e} M. Amp{\`e}re\IMSauthorrefmark{\#3}
}% end of \IMSauthorblockNAME
\\%
%
\IMSauthorblockAFFIL{% Author Affiliations
\IMSauthorrefmark{\#}My Lab, My University, USA\\
\IMSauthorrefmark{\$}Gollum World, New Zealand\\
\IMSauthorrefmark{*}My Research, Australia\\
\IMSauthorrefmark{\textasciicircum}SolarSolar, Australia
}% end of \IMSauthorblockAFFIL
\\%
%
\IMSauthorblockEMAIL{% Author Emails
\IMSauthorrefmark{1}J.Clerk.Maxwell@my-uni.edu, \IMSauthorrefmark{2}michael@mr.com.au, \IMSauthorrefmark{3}Ampere@my-uni.edu
...
or spacing to meet the page limit. You must not change the required
formats to meet the page limit.
...
Causal Productions, Australia). Do not show physical address, phone
...
Note that a period is used to properly punctuate the previous sentence.
It is placed at the end of the second equation. Make sure that all parts of
...
number, as in [2]. Do not use ``Ref. [3]'' or ``Reference [3]'' except at
the beginning of a sentence, e.g. ``Reference [3] shows \ldots''. Multiple
...
the text to be formatted, then select an appropriate style in the
``Styles'' task pane.
...
excluding the margins. Text box must not be used with captions because
it is difficult to set the same caption width in all captions with
text box. Simply type the caption below the figure or above the table
and then format the caption using an appropriate style from Table 2.
...
this template is V4c. The format of this template was adapted by Causal
Productions mainly from the IEEE LaTeX class file. There are important
differences between the IEEE format and the IMS format, so IMS
authors must use only the latest version of the IMS templates.

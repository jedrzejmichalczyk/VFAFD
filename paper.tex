...
Rational approximation in the Hardy space $H^2(\mathbb{D})$ is fundamental to signal processing, control theory, and microwave engineering. Two prominent methods exist: Adaptive Fourier Decomposition (AFD) and Vector Fitting (VF). AFD employs greedy selection with orthogonal Blaschke products, guaranteeing numerical stability but potentially suboptimal pole placement. VF uses iterative pole relocation via least squares, achieving better approximations but suffering from ill-conditioned linear systems.
